...
public class TestGraph extends JApplet {
	...
	public int[] paintGraph(int a) {//a表示某一个template
		...
		//测试数据
			...
			List<NodeBean> list=new ArrayList<NodeBean>();//点的信息
			List<EdgeBean> List=new ArrayList<EdgeBean>();//边的信息
		...
		while(i<Vertexcount)
		{
 			list.add(new NodeBean(VertexID[i]));//添加node
 			i++;
		}
			for( I=0;I<Vertexcount;I++)			
			{	
				for(J=0;J<Vertexcount;J++)
					if(A[I][J]==1&&A[I][J]!=A[J][I]&&J!=I)//找到存在的边,并且不是对角线上的edge
		           {      //添加edge
						List.add(new EdgeBean(list.get(I),list.get(J),new Long(100)));//先加
		        	}
					else if(A[I][J]==1&&A[I][J]==A[J][I]&&J!=I&&B[I]!=1)
					{//存在回路的边用B[]记录一下 用flag记录一下回路边的数目
						List.add(new EdgeBean(list.get(I),list.get(J),new Long(100)));//后加
						B[I]=B[J]=1;
						flag++;
					}
...
//						System.out.println(EdgePosition[L]);			
	...
	private void addEdgeGef(NodeBean source, NodeBean target, int weight, String edgeString) {
		...
		if(source.equals(target)){//如果开始的点==终止的点
			return;
		}
		// 建立GEF的 node edge将来用来计算graph node的layout
