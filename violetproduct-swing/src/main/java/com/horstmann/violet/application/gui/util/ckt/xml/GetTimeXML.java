...
//		System.out.println("--------======");
	...
	public static void produceXML(String path, ArrayList<Automatic> testcaselist) {
		...
		System.out.println(testcaselist.size() + "-----测试路径的个数");
		// 1、创建document对象，代表整个xml文档
		Document dom = DocumentHelper.createDocument();
		// 2、创建根节点TCS
		org.dom4j.Element tcs = dom.addElement("TCS");
		...
		for (int i1 = 0; i1 < testcaselist.size(); i1++) {
			// 4、生成子节点及节点内容
			Element testcase = tcs.addElement("testcase");
			for (int j = 0; j < testcaselist.get(i1).getTransitionSet().size(); j++) {
				// 添加节点
				Element process = testcase.addElement("process");
				Element operation = process.addElement("operation");
				operation.setText(testcaselist.get(i1).getTransitionSet().get(j).getName());
				Element input = process.addElement("input");
				int random = -1;
				if (random == -1) {
					random = new Random().nextInt(testcaselist.get(i1).getTransitionSet().get(j).getResult().size());
				}
				input.setText(testcaselist.get(i1).getTransitionSet().get(j).getResult().get(random));
				Element time = process.addElement("time");
				time.setText(testcaselist.get(i1).getTransitionSet().get(j).getTranTimeName());
			}
			Element limit = testcase.addElement("limit");
			Element operation = limit.addElement("operation");
			String s = null;
