...
public class TestCasePartPanel extends JPanel {
	...
	private void showData() {
		...
		Object[] rowData1 = { "序号", "名称", "概率", "条件", "预期结果" };
		attributeTableModel.addRow(rowData1);
		for (Transition transition : route.getTransitionList()) {
			Object[] rowData = { transition.getId(), transition.getName(), transition.getProbability(),
					transition.getResultOfCondition(), transition.getExpectResult() };
	...
	private void initAttributeTablePanel() {
		...
		final String[] columnNames = { "序号", "名称", "概率", "条件", "预期结果" };
		String[][] tabelValues = {};
		...
		attributeTable.getColumn("概率").setPreferredWidth(60);
		attributeTable.getColumn("概率").setMinWidth(60);
		attributeTable.getColumn("概率").setMaxWidth(60);
