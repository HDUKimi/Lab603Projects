...
public class PerformanceXML {
	public static void main(String[] args) throws Exception {
		...
		// 6、生成xml文件
		File file = new File("E:\\XML\\UAVForXStreamGAODU+border+path+perform.xml");
	...
	public static List<List<String>> cases = new ArrayList<List<String>>(); // 测试用例集合
	...
	public static void produceXML(List<List<String>> listcases, String path){
...
//		System.out.println("第"+i+"条测试路径上解个数"+numm);
		...
		for(int n1=0;n1<num;n1++){
			...
			Element testcase = tcs.addElement("testcase");
			//System.out.println("---------------------testcase"+n1);
			for(int nn=0;nn<listcases.size();nn++){//cases.size表示边的个数
				//添加节点
				Element process = testcase.addElement("process");
				Element operation = process.addElement("operation");			
				int random = -1;
				if (random == -1) {
					random = new Random().nextInt(listcases.get(nn).size());
				}
				//System.out.println("random-->"+random);
				String value = listcases.get(nn).get(random);
				//System.out.println("解value-->"+value);
				String[] cs =value.toString().split("%");
				//System.out.println("operation-->"+cs[0]);
				if(cs[0].contains("flag=1")){
