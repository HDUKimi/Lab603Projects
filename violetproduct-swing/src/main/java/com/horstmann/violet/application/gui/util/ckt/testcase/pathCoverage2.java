...
public class pathCoverage2 {
	private static Automatic mAutomatic;
	private static Automatic a;	
	public static int maxCircle = 4;// 最少环走一次
	...
	public static void main(String[] args) {
		...
		dfs(initialState);
		ArrayList<Automatic> testcaseSet = testCase1(TT);
		System.out.println("测试路径个数："+testcaseSet.size());
		for(int i=0;i<testcaseSet.size();i++){
			System.out.println("第"+(i+1)+"条测试路径");
			for(Transition tran:testcaseSet.get(i).getTransitionSet()){
	...
	public static ArrayList<Automatic> testCase(Automatic aaa){
		//搜索终止状态集合
		for(State state:aaa.getStateSet()) {
			int k1= 0;
		...
		visitedState.add(intstate);
		
		
		while(!sstack.isEmpty()){
			State X=sstack.peek();//获得栈顶元素，但不出栈
			nextTransition = nextTran(X);//获得与状态连接的一条迁移
			//System.out.println(nextTransition.getName()+"-------=====------");
			if(nextTransition == null){
				if(X.getName().equals(intstate.getName())){
					sstack.pop();
				}else{
					sstack.pop();
					tstack.pop();
				...
				nextTransition.setVisited(true);//将这条迁移标志位已访问过
				tstack.push(nextTransition);//将这条迁移入栈
				for(State s:aaa_StateSet){
					if(nextTransition.getTarget().equals(s.getName())){//找到这条迁移的目标状态
						sstack.push(s);//目标状态入栈；
						visitedState.add(s);//设置目标状态被访问
	...
	private static void dfs(State initialState) {
		...
		ArrayList<Transition> a_TranSet=a.getTransitionSet();//获得时间自动机状态集合	
		/*//搜索终止状态集合
		for(State state:a.getStateSet()) {
			int k1= 0;
			for(Transition tran:a.getTransitionSet()){//判断目标状态是否已被访问
				if(state.getName().equals(tran.getSource())){//找出以此状态为起点的迁移
					k1=1;
				}
			}
	...
	public static ArrayList<Automatic> testCase1(ArrayList<ArrayList<Transition>> TT){
