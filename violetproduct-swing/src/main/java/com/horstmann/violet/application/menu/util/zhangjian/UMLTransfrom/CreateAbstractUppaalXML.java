...
public class CreateAbstractUppaalXML {
	...
	private void getLayoutUppaalInfo() throws DocumentException {
		...
		for (Element element : locations) {
			...
			locationmap.put(element.attributeValue("id"),
					element.attributeValue("x") + "-" + element.attributeValue("y"));
		}
		for (Element element : transitions) {
			...
			transitionmap.put(element.attributeValue("id").split("tran_id")[1],
					element.element("label").attributeValue("x") + "-" + element.element("label").attributeValue("y"));
		}
	}
...
//			System.out.println("*************************" + j + "   " + abTransitionList.size());
...
//			System.out.println("*************************");
			...
			// TransitionEdge.addElement("start").addAttribute("class",
			// XMLUtils.getState(sourceID,abNodeList).getType()).addAttribute("reference",
			// XMLUtils.getMapId(map, sourceID));
			// TransitionEdge.addElement("end").addAttribute("class",
			// XMLUtils.getState(targetID,abNodeList).getType()).addAttribute("reference",
			// XMLUtils.getMapId(map, targetID));

			TransitionEdge.addElement("start").addAttribute("class", "CircularNode").addAttribute("reference",
					XMLUtils.getMapId(map, sourceID));
			TransitionEdge.addElement("end").addAttribute("class", "CircularNode").addAttribute("reference",
					XMLUtils.getMapId(map, targetID));

			TransitionEdge.addElement("startLocation").addAttribute("class", "Point2D.Double")
					.addAttribute("id", k + 1 + "").addAttribute("x", x).addAttribute("y", y);
			...
			// String(edge.getSource().getBytes(),"utf-8")
			// +" target："+edge.getTarget()//new
			// String(edge.getTarget().getBytes(),"utf-8")
			// +" ResetClockSet："+edge.getResetClockSet()//new
			// String(edge.getResetClockSet().getBytes(),"utf-8")
			// +" DBM："+edge.getConstraintDBM());//new
			// String(edge.getConstraintDBM().getBytes(),"utf-8"));

			// TransitionEdge.addElement("labelText").addText("");

			// TransitionEdge.addElement("labelText").addText("type:"+edge.getType()
			// +" ResetClockSet："+edge.getResetClockSet()
			// +" DBM："+edge.getConstraintDBM());
...
//			TransitionEdge.addElement("labelText").addText(edge.getTranTimeName() + "");
	 ...
	 * @param root
	 */
	private void createState(String backgroundid, String borderid, String textid, int k, Element root) {
...
//				System.out.println("123456798123456789");
//
//				Element cStartNode = nodes.addElement("CircularNode").addAttribute("id", k + "");
//
//				map.put(node.getSid() + "", k + "");
//
//				cStartNode.addElement("children").addAttribute("id", k + 1 + "");
...
//				} else {
//					Element backcolor = cStartNode.addElement("backgroundColor").addAttribute("reference",
//							backgroundid);
...
//			System.out.println();
			if ("Start".equals(node.getType().trim())) {
				...
				cNode.addElement("location").addAttribute("class", "Point2D.Double").addAttribute("id", k + 2 + "")
				...
				// name.addText("
				// name:"+node.getSname()+";DBM:"+node.getInvariantDBM()+";position:"+node.getPosition());
				// name.addText("");
				// name.addText(node.getSname());
				name.addText(node.getSid() + "");
