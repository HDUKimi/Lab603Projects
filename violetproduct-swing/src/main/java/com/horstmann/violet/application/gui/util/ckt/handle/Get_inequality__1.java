...
public class Get_inequality__1 {
	/**
	 * 获得所有时间 自动机的不等式组
	 * @param as
	 * @return
	 */
	public static ArrayList<ArrayList<String>> get_AllInequalitys(ArrayList<Automatic> as){
		ArrayList<ArrayList<String>> all_inequalitys=new ArrayList<ArrayList<String>>();
		for(Automatic a:as){
		...
		if(resetNumber==1&&clock_number==2){
			...
			for(Transition t1:TransitionSet){
				...
				if(flag==true){//第一条边到该边的边的集合中有时钟复位
					...
					for(Transition ts:Ts){
						if(ts.getEventSet().size()==1){
							label1.add(ts.getEventSet().get(0));
						}
						else if(ts.getEventSet().size()==2){
