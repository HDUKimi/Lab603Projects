...
public class WriteVioletUppaal {
    	...
    	int i=0; 
    	int j=0;
    	//读取stabilize_run.xml(主要是读取我们想要的信息)(解析生成事件自动机的xml)
    	public void find() throws DocumentException{
    		...
    		for(Element temp : transitions){
    			...
    			if(temp.element("label")!=null){
        			...
        			System.out.println("++++++++++++++++++++++++++++++++++++++++++++"+temp.elements("label").size());
        			String labeltext=" ";
        			for(int i=0;i<temp.elements("label").size();i++){
        				System.out.println(((Element)temp.elements("label").get(i)).getText());
        				labeltext+=((Element)temp.elements("label").get(i)).getText();
        				labeltext+=" ";
        			}
