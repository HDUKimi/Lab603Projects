...
public class Read
{
	...
	static ArrayList<WJDiagramsData> umlAllDiagramData = new ArrayList<WJDiagramsData>();
	ArrayList<REF> umlREF = new ArrayList<REF>();
	static ArrayList<WJDiagramsData> displayDiagrams = new ArrayList<WJDiagramsData>();//展示用的图
...
//4444444444444444444444444444444444444			
			...
			message.setTargetId(EAmessage.getTragetId());		//targetid
			...
			for(Iterator<WJFragment> fragIterator=umlFragment.iterator();fragIterator.hasNext();)
			{
				...
				for(int i=0; i< fragment.getSourceId().length;i++)//看message在哪个fragment中
					if(message.getSourceId().equals(s[i]) && message.getTargetId().equals(t[i]))
					{	
...
/*method-----------------------------------------------------------*/	
	public static void DFSDiagramByREF(WJDiagramsData diagramData) {
	...
	private static void fixDiagramData(WJDiagramsData diagramData, WJDiagramsData childDiagram, REF ref) {
		//添加lifeline
		...
		if (ref.index > diagramData.getMessageArray().size()) {
			
			System.err.println("ref插入位置大于图的消息个数");
			System.err.println("ref.index = " + ref.index + "; " + "messageSize = " + diagramData.getMessageArray().size());
			System.err.println("图名：" + diagramData.getName());
		}
...
//		}
	...
	private static void addMessageMarkId(WJMessage message) {
	...
	private static void addFragmentMarkId(WJFragment copyFragment) {
	...
	private static WJDiagramsData findDiagramByName(String diagramName) {
