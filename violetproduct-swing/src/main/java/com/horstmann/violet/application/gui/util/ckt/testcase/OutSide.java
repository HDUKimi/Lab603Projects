...
public class OutSide {
	...
	public static void main(String[] args) {
		...
		//getBorder(s);
		String s = "!(elevator_thisFloorOut == true)#elevator_thisFloorOut:bool--!(getCurrentFloor_empty == false)#getCurrentFloor_empty:bool";
		String[] ss1 = s.split("--");
	...
	public static String getOutSide(String domain1) {
		...
		for(int i=0;i<bds.length;i++){
			...
			if(bds[i].contains("!")){
				System.out.println("含有！符号"+bds[i]);
				String s = null;
				if(bds[i].substring(0, 1).toString().contains("!")){
					s = bds[i].replace("!", "");
				}else{
				    s = bds[i].replace("!", "=");
				}
				...
				//System.out.println();
				//System.out.println("======");
				if(bds[i].contains("True") || bds[i].contains("true")){
					String s1 = bds[i].replace("True", "false").replace("true", "false").replace("==", "=");
					bdsSet.add(s1);
				}else{
					if(bds[i].contains("False") || bds[i].contains("false")){
						String s1 = bds[i].replace("False", "true").replace("false", "true").replace("==", "=");
						bdsSet.add(s1);
					}else{
						if(getNumber(bds[i])){
							System.out.println("=====" + bds[i]+"=====");
