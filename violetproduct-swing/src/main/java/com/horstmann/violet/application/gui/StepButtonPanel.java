...
import com.horstmann.violet.application.gui.util.wujun.SequenceTransfrom.WJDiagramsData;
							...
							try {
								...
								if (name.contains("EA")) {// 打开ea平台的xml文件
									if(name.contains("seq")){
										...
										//String diagramName = new WJDiagramsData().getName() + ".xml";// 取出生成xml的名称(wj生成的提供给我的)
										...
										LayoutUppaal.layout("UseCase1-Sequence1-Normal.xml");//("sequence.xml");
										...
										LayoutUppaal.layout("1.xml");//("sequence.xml");
										String filename1 = TransToVioletUppaal.TransToViolet();
										// String
										// filename1="uppaalTest1.uppaal.violet.xml";
										GraphFile fGraphFile1 = ImportByDoubleClick.importFileByDoubleClick("UPPAAL",
			...
			public void actionPerformed(ActionEvent e) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						 ...
						 ClientSocket("192.168.150.152",5555);
...
//								tc.setDetail(str);
//								tc.setContent(
//										"激励参数 : throttle_failsafe=0,failsafe_throttle_value=930   激励状态 : 103 ) read_radio (    激励参数 : null   激励状态 : 144 ) set_has_new_input (    激励参数 : new_input=1   激励状态 : 6 ) set_last_update_radio_ms (    激励参数 : null   激励状态 : 6 ) set_pwm (    激励参数 : roll_pwm=10,pitch_pwm=10,throttle_pwm=10,yaw_pwm=10   激励状态 : 12 ) get_failsafe_throttle_enable (    激励参数 : null   激励状态 : 5 ) set_throttle_zero_flag (    激励参数 : null   激励状态 : 10 )"
//												);
//								tc.setState((int)(Math.random()*100)+"");
//								tc.setResult("成功");
//							list.add(tc);
//						}
						// System.out.println(list.size()+"测******试");
//						for (TestCase tc : list) {
//							System.out.println(tc.getTestCaseID() + "  " + tc.getContent() + " " + tc.getState() + "  "
//									+ tc.getResult());
//						}
						// mainFrame.getStepFiveCenterTabbedPane().getTestcaseFile().setLayout(new
						// GridLayout(1, 1));
						// 获得root的容器
	...
	public JTextArea getStepTwoArea() {
		return StepTwoArea;
	}
