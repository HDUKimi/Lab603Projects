...
public class StepButtonPanel extends JPanel {
	...
	// StepSixCenterTabbedPane stepSixCenterTabbedPane=new
	// StepSixCenterTabbedPane();
	...
	private void initButton() {
		...
		// icon.setImage(icon.getImage().getScaledInstance(22, 22,
		// Image.SCALE_DEFAULT));
		...
		// TODO Auto-generated method stub
		step2button.setEnabled(true);// 初始其他步骤按钮都不可点击
	...
	private void SetButtonListener() {
						...
						public void run() {
							try {
								...
								if (name.contains("EA")) {// 打开ea平台的xml文件
									...
									GraphFile fGraphFile1 = ImportByDoubleClick.importFileByDoubleClick("UPPAAL",
											filename1);
			...
			public void actionPerformed(ActionEvent e) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						// GetAutomatic.getAutomatic("Draw
						// MoneyForXStream(2).xml");
						// by tan 将抽象测试用例的state加入到数据库中
						// new
						// AbstractStateInsertByTan().wqq2zhangExchange("Draw
						// MoneyForXStream(2).xml");
						// BY TAN 强抽象测试的迁移trasition加入到数据库中
						// new
						// AbstractTrasitionAndStateInsertByTan().w2zExchange("Draw
						// MoneyForXStream(2).xml");
						...
						try {
							...
							new AbstractTestCaseUppaalSequenceCreate().createXML(
									"D:\\ModelDriverProjectFile\\UPPAL\\2.UML_Model_Transfer\\UseCase1-Sequence1-NormalForXStream.xml");
							LayoutUppaal.layout(absolutePath + "\\sequence.xml");
						} catch (Exception e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
						...
						System.out.println("-----------------------------------");
						// 将信息存入数据库
						new AbstractTestCaseUppaalCreate().createXML(
								"D:\\ModelDriverProjectFile\\UPPAL\\2.UML_Model_Transfer\\UseCase1-Sequence1-NormalForXStream.xml",
								"D:\\ModelDriverProjectFile\\UPPAL\\2.UML_Model_Transfer\\abs.uppaal.violet.xml");
						// new
						// AbstractTestCaseInsertByTan().w2zAbstractTestCaseExchange("D:\\ModelDriverProjectFile\\UPPAL\\2.UML_Model_Transfer\\sequenceForXStream.xml");//已经整合到上面的方法中
						// String absfilename="abs.uppaal.violet.xml";
						// String
						// no_time_absfilename="no_time_abs.uppaal.violet.xml";

						// String filename2=TransToVioletUppaal.TransToViolet();

						GraphFile absfGraphFile = ImportByDoubleClick.importFileByDoubleClick("UPPAAL",
								"abs.uppaal.violet.xml");
						// GraphFile
						// no_time_absfGraphFile=ImportByDoubleClick.importFileByDoubleClick("UPPAAL","a.xml");
						IWorkspace absworkspace = new Workspace(absfGraphFile);
						// IWorkspace no_time_absworkspace=new
						// Workspace(no_time_absfGraphFile);
						// 展示时间迁移的自动机
						...
						// 展示抽象测试用例的生成
						stepThreeCenterTabbedPane.getConsolePartScrollPane().getViewport()
								.add(new ConsolePartDetailInfoTable(0));
			...
			public void actionPerformed(ActionEvent e) {
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						stepFourCenterTabbedPane.getConsolePartScrollPane().getViewport()
								.add(new ConsolePartDetailInfoTable(1));
		...
		Sixstart.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					// mainFrame.getStepSixCenterTabbedPane().getConsolePartScrollPane().setLayout(new
					// BorderLayout());
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (firstclickstep1button == 0) {
					...
					for (int i = 0; i < count; i++) {
						// ((ButtonTabbedPanel)
						// mainFrame.getStepOneCenterTabbedPane().getButtonPanel().getComponent(i)).getTabbedbutton();
						...
						if (ob instanceof ButtonTabbedPanel) {
							...
							// mainFrame.getStepOneCenterTabbedPane().getButtonPanel().remove((ButtonTabbedPanel)
							// ob);
				...
				mainFrame.getAttributePartPanel().getNamepanel()
						.add(mainFrame.getAttributePartPanel().getSteponenamelabel());
				...
				// fileNamingService.getFileFilters();
				...
				// //增加转换的方法11
				// selectedFile= new
				// FileMenu(mainFrame).exchangeFile(selectedFile, graphFile ,
				// flag);
				//
				// IFileReader file1 =
				// fileChooserService.getFileReader(selectedFile);
				// InputStream in = file1.getInputStream();
				// //通过流获得文件的内容
				// jta.append(new
				// XHTMLPersistenceService().getInputStreamContent(in));
				...
				// e1.printStackTrace();
				// }
				// consolePart.add(new ConsoleMessageTabbedPane("详细信息",new
				// JTextArea("UML模型正在建立中......\n\n\n\n\n\n")));
				wakeupUI();
				mainFrame.setVisible(false);
				mainFrame.getContentPane().repaint();
				mainFrame.setVisible(true);
		...
		step2button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				mainFrame.getAttributePartPanel().getNamepanel().removeAll();
				mainFrame.getAttributePartPanel().getNamepanel()
						.add(mainFrame.getAttributePartPanel().getSteptwonamelabel());
		...
		step3button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// consolePart.add(new
				// ConsoleMessageTabbedPane("详细信息",StepThreeArea));
				...
				mainFrame.getAttributePartPanel().getNamepanel().removeAll();
				mainFrame.getAttributePartPanel().getNamepanel()
						.add(mainFrame.getAttributePartPanel().getStepthreenamelabel());
		...
		step4button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// consolePart.add(new
				// ConsoleMessageTabbedPane("详细信息",StepFourArea));
		...
		step5button.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// consolePart.add(new
				// ConsoleMessageTabbedPane("详细信息",StepFiveArea));
