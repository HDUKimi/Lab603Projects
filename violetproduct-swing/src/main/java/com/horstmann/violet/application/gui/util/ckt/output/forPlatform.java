...
public class forPlatform {
	...
	public static Automatic plat4(Automatic a) {
		...
		// Automatic automatic=IPR__1.iPR(a);//获得拆分后的时间自动机
		// Automatic
		// aTDRTAutomatic=ATDTR__1.aTDRT(automatic,a);//获得去除抽象时间迁移后的时间自动机
	 ...
	 * 
	 * @param a
	 *            传入要进行状态覆盖的时间自动机，不是进行DFSTree的时间自动机
	 * @return
	 */
	...
	/**
	 * 处理测试序列，得到抽象测试用例ArrayList<Automatic> 把迁移上的不等式组合起来，放入limit属性中
	 * 
	 * @param xml
	 * @return
	 */
	...
	public static ArrayList<Automatic> plat8(ArrayList<Automatic> collectLimit) {
		...
		// collectLimit(testcase);//把测试用例不等式组合一起，写入时间自动机
